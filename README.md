...
I began by exploring the dataset using SQL to understand its structure and key metrics, including revenue trends, product performance and average transaction value.
...
| Metric | Definition | Calculation | Verified Value |
|---|---|---|---:|
| Recorded Transactions | Number of distinct transaction IDs in the supplied dataset | `COUNT(DISTINCT transaction_id)` | $149,116 |
| Items Sold | Total number of individual units sold | `SUM(transaction_qty)` | $214,470 |
| Revenue | Total sales value generated by all recorded transactions | `SUM(transaction_qty * unit_price)` | $698,812.33 |
| Average Transaction Value | Average revenue generated per recorded transaction ID | `SUM(transaction_qty * unit_price) / COUNT(DISTINCT transaction_id)` | $4.69 |
...
### Peak Sales Hours
- Investigate staffing requirements during peak morning periods by comparing transaction volumes with staffing levels, service times and queue lengths. If service performance deteriorates during these periods, additional staffing could then be tested.
...
### Average Transaction Value
- Explore whether product bundling could increase transaction value by first analysing the number and combination of items purchased per transaction. A bundle promotion could then be tested and its effect on transaction value measured.
